function Invoke-StopRemoteServiceJob
{
    [CmdletBinding()]
    Param
    (
    ...
    Begin
    {
        Write-Verbose "Started Invoke-StopRemoteServiceJob."
        Write-Progress -Id 2 -ParentId 1 -Activity "Stopping Services" -Status "Starting..." -PercentComplete 0.00
    }
    Process
    {
        try
        {
            ...
            foreach ($Server in $Servers)
            {
                ...
                Write-Progress -Id 2 -ParentId 1 -Activity "Stopping Services" -Status "Running Jobs. $($JobCount) total. $($CompletedJobCount) completed. $($FailedJobCount) failed." -PercentComplete $PercentComplete
                ...
                if (($JobsStarted | Where-Object State -eq "Running").Count -lt $ThrottleLimit)
                {
                    $ScriptBlock = {
                        ...
                        try
                        {
                            function Stop-RemoteService
                            {
                                ...
                                try
                                {
                                    ...
                                    while ($Service.State -ne "Stopped" -and $Stopwatch.Elapsed -lt $Timeout)
                                    {
                                        ...
                                        {
                                            Write-Verbose "Stopping the $($ServiceName) service on $($ServerName) using CIM."
                                ...
                                catch
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        while ($Service.State -ne "Stopped" -and $Stopwatch.Elapsed -lt $Timeout)
                                        {
                                            if ($Service.State -ne "Stopping")
                                            {
                                                Write-Verbose "Stopping the $($ServiceName) service on $($ServerName) using WMI."
                                    ...
                                    {
                                        Write-Error "An error occurred while ensuring the $($ServiceName) service was stopped on $($ServerName): $($_.Exception.Message)"
                                        throw
                                    }
                                }
                                finally
                            ...
                            Stop-RemoteService -ServerName $ServerName -ServiceName $ServiceName
                        }
                        catch
                        {
                    ...
                    Write-Verbose "Running job to stop the '$($ServiceName)' service on $($Server.Name)."
    ...
    End
    {
        Write-Progress -Id 2 -ParentId 1 -Activity "Stopping Services" -Status "Completed" -Completed
        Write-Verbose "Completed Invoke-StopRemoteServiceJob"
    }
}